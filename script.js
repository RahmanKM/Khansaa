...
button.addEventListener("click", () => {
  if (button.textContent === "Suck my dick please? ❤") {
    button.textContent = "loading...";
    fetch('send_mail.php')
      .then(response => {
        if (response.ok) {
          button.textContent = "you just gave me the best orgasm 🙃";
        } else {
...
function drawText() {
    ...
    if(frameNumber < 250){
        ...
        context.fillText("Jaan everyday day I cannot believe how lucky I am", canvas.width/2, canvas.height/2);
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much Khansaa, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much Khansaa, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
    ...
    if(frameNumber >= 3000 && frameNumber < 99999){
        ...
        context.fillText("Happy Valentine's Day Jaan <3", canvas.width/2, (canvas.height/2 + 120));
